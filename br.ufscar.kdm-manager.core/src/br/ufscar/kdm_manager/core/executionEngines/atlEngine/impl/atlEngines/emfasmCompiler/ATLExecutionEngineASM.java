 ...
 *
 */
public class ATLExecutionEngineASM extends ATLExecutionEngine<Map<String, String>, Object[]>{
	...
	private Map<String, Object[]> outModels = null;
		...
		try {
			...
			for (String metamodelsModelsInName : modelsInData.keySet()) {
				for (String modelsInName : modelsInData.get(metamodelsModelsInName).keySet()) {
					...
					launcher.addInModel(inModel, modelsInName, metamodelsModelsInName);
				}
			}
			for (String metamodelsModelsOutName : modelsOutData.keySet()) {
				for (String modelsOutName : modelsOutData.get(metamodelsModelsOutName).keySet()) {
					IModel outModel = factory.newModel(metamodels.get(metamodelsModelsOutName));
					//injector.inject(outModel, modelsOutData.get(metamodelsModelsOutName).get(modelsOutName));
					
					Object modelData[] = new Object[2];
					modelData[0]=outModel;
					modelData[1]=modelsOutData.get(metamodelsModelsOutName).get(modelsOutName);
					outModels.put(modelsOutName, modelData);

					launcher.addOutModel(outModel, modelsOutName, metamodelsModelsOutName);
				}
		...
		try {
			if(atlConfigured){
				...
				for (String outModelName : outModels.keySet()) {
					IModel model = ((IModel)outModels.get(outModelName)[0]);
					System.out.println(outModelName);
					System.out.println(model);
					System.out.println(((String)outModels.get(outModelName)[1]));
					extractor.extract(model , ((String)outModels.get(outModelName)[1]));
				}
				return true;
			}
		} catch (ATLCoreException e) {
